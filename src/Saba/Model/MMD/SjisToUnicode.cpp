...
{
	if (ASCIIBegin <= ch && ch <= ASCIIEnd)
	{
		return true;
	}
	else if (HankakuBegin <= ch && ch <= HankakuEnd)
	{
		return true;
	}
	return false;
...
int ConvertSjisToUnicode(int ch)
{
	...
	if (ASCIIBegin <= ch && ch <= ASCIIEnd)
	{
		unicode = AsciiTable[ch];
	}
	else if (HankakuBegin <= ch && ch <= HankakuEnd)
	{
		unicode = HankakuTable[ch - HankakuBegin];
	}
	else
	{
		int code1 = ch >> 8;
			...
			)
		{
			...
			code2 -= SjisSecondBegin;
			unicode = SjisTable2[code1][code2];
		}
	}
	return unicode;
}
...
std::wstring ConvertSjisToWString(const char* sjisCode)
{
	...
	while (*sjisChar != 0)
	{
		if (IsSjis1ByteChar((uint8_t)*sjisChar))
		{
			sjisChar += 1;
			chCount++;
		}
	...
	std::wstring wstr(chCount, L'\0');
	sjisChar = sjisCode;
	auto wstrData = wstr.begin();
	while (*sjisChar != 0)
	{
		if (IsSjis1ByteChar((uint8_t)*sjisChar))
		{
			*wstrData = (wchar_t)ConvertSjisToUnicode((uint8_t)*sjisChar);
			sjisChar += 1;
			wstrData++;
		}
		else
		{
			int code1 = (uint8_t)*sjisChar;
			sjisChar++;
			int code2 = (uint8_t)*sjisChar;
			sjisChar++;
			int ch = (code1 << 8) | code2;
			auto unicode = ConvertSjisToUnicode(ch);
			if (unicode == 0xFFFF)
			{
				return L"";
			}
			*wstrData = (wchar_t)unicode;
			wstrData++;
		}
	}
